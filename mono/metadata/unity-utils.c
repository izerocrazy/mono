...
MONO_API void
mono_unity_set_embeddinghostname (const char *name)
{
	gEmbeddingHostName = g_string_new (name);
}

MonoString *
mono_unity_get_embeddinghostname ()
{
...
void
mono_unity_set_vprintf_func (vprintf_func func)
{
...
unsigned
mono_unity_get_all_classes_with_name_case (MonoImage *image, const char *name, MonoClass **classes_ref, unsigned *length_ref)
{
	MonoClass *klass;
...
void
mono_unity_object_init (void *obj, MonoClass *klass)
{
	if (klass->valuetype)
...
MonoObject *
mono_unity_object_isinst_sealed (MonoObject *obj, MonoClass *targetType)
{
	return obj->vtable->klass == targetType ? obj : NULL;
}

void
mono_unity_object_unbox_nullable (MonoObject *obj, MonoClass *nullableArgumentClass, void *storage)
{
...
MonoClass *
mono_unity_object_get_class (MonoObject *obj)
{
	return obj->vtable->klass;
}

MonoObject *
mono_unity_object_compare_exchange (MonoObject **location, MonoObject *value, MonoObject *comparand)
{
	return ves_icall_System_Threading_Interlocked_CompareExchange_T (location, value, comparand);
}

MonoObject *
mono_unity_object_exchange (MonoObject **location, MonoObject *value)
{
	return ves_icall_System_Threading_Interlocked_Exchange_T (location, value);
}

gboolean
mono_unity_object_check_box_cast (MonoObject *obj, MonoClass *klass)
{
...
const char *
mono_unity_class_get_image_name (MonoClass *klass)
{
	return klass->image->assembly_name;
}

MonoClass *
mono_unity_class_get_generic_definition (MonoClass *klass)
{
...
MonoClass *
mono_unity_class_inflate_generic_class (MonoClass *gklass, MonoGenericContext *context)
{
	MonoError error;
...
gboolean
mono_unity_class_has_parent_unsafe (MonoClass *klass, MonoClass *parent)
{
	return mono_class_has_parent_fast (klass, parent);
}

MonoAssembly *
mono_unity_class_get_assembly (MonoClass *klass)
{
	return klass->image->assembly;
}

gboolean
mono_unity_class_is_array (MonoClass *klass)
{
	return klass->rank > 0;
}

MonoClass *
mono_unity_class_get_element_class (MonoClass *klass)
{
	return klass->element_class;
}

gboolean
mono_unity_class_is_delegate (MonoClass *klass)
{
	return klass->delegate;
}

int
mono_unity_class_get_instance_size (MonoClass *klass)
{
	return klass->instance_size;
}

MonoClass *
mono_unity_class_get_castclass (MonoClass *klass)
{
	return klass->cast_class;
}

guint32
mono_unity_class_get_native_size (MonoClass *klass)
{
...
MonoBoolean
mono_unity_class_is_string (MonoClass *klass)
{
...
MonoBoolean
mono_unity_class_is_class_type (MonoClass *klass)
{
...
gboolean
mono_unity_class_has_cctor (MonoClass *klass)
{
...
MonoMethod *
mono_unity_method_get_generic_definition (MonoMethod *method)
{
...
mono_unity_method_get_object (MonoMethod *method)
{
	MonoError unused;
...
MonoMethod *
mono_unity_method_alloc0 (MonoClass *klass)
{
	return mono_image_alloc0 (klass->image, sizeof (MonoMethod));
}

MonoMethod *
mono_unity_method_delegate_invoke_wrapper (MonoClass *klass)
{
...
gboolean
mono_unity_method_is_static (MonoMethod *method)
{
	return method->flags & METHOD_ATTRIBUTE_STATIC;
}

MonoClass *
mono_unity_method_get_class (const MonoMethod *method)
{
	return method->klass;
}
...
void *
mono_unity_method_get_method_pointer (MonoMethod *method)
{
	return method->method_pointer;
}

void
mono_unity_method_set_method_pointer (MonoMethod *method, void *p)
{
	method->method_pointer = p;
}

void *
mono_unity_method_get_invoke_pointer (MonoMethod *method)
{
	return method->invoke_pointer;
}

void
mono_unity_method_set_invoke_pointer (MonoMethod *method, void *p)
{
...
const char *
mono_unity_method_get_name (const MonoMethod *method)
{
	return method->name;
}
...
static guint32
get_array_structure_hash (MonoArrayType *atype)
{
	char buffer[100];
	char *ptr = buffer;
...
static void
get_type_hashes (MonoType *type, GList *hashes, gboolean inflate);
static void
get_type_hashes_generic_inst (MonoGenericInst *inst, GList *hashes, gboolean inflate);
...
static GList *
get_type_hashes_method (MonoMethod *method, gboolean inflate)
{
...
static guint64
combine_hashes (guint64 hash1, guint64 hash2)
{
...
static void
combine_all_hashes (gpointer data, gpointer user_data)
{
...
guint64
mono_unity_method_get_hash (MonoMethod *method, gboolean inflate)
{
...
guint64
mono_unity_type_get_hash (MonoType *type, gboolean inflate)
{
	GList *hashes = monoeg_g_list_alloc ();
...
MonoMethod *
mono_unity_method_get_aot_array_helper_from_wrapper (MonoMethod *method)
{
	MonoMethod *m;
	const char *prefix;
...
MonoObject *
mono_unity_method_convert_return_type_if_needed (MonoMethod *method, void *value)
{
...
guint32
mono_unity_method_get_token (MonoMethod *method)
{
	return method->token;
}
...
void
mono_unity_domain_install_finalize_runtime_invoke (MonoDomain *domain, RuntimeInvokeFunction callback)
{
	domain->finalize_runtime_invoke = callback;
}

void
mono_unity_domain_install_capture_context_runtime_invoke (MonoDomain *domain, RuntimeInvokeFunction callback)
{
	domain->capture_context_runtime_invoke = callback;
}

void
mono_unity_domain_install_capture_context_method (MonoDomain *domain, gpointer callback)
{
...
int
mono_unity_array_get_element_size (MonoArray *arr)
{
	return arr->obj.vtable->klass->sizes.element_size;
}

MonoClass *
mono_unity_array_get_class (MonoArray *arr)
{
	return arr->obj.vtable->klass;
}

mono_array_size_t
mono_unity_array_get_max_length (MonoArray *arr)
{
...
gboolean
mono_unity_type_is_generic_instance (MonoType *type)
{
	return type->type == MONO_TYPE_GENERICINST;
}

MonoGenericClass *
mono_unity_type_get_generic_class (MonoType *type)
{
	if (type->type != MONO_TYPE_GENERICINST)
		return NULL;
...
gboolean
mono_unity_type_is_enum_type (MonoType *type)
{
...
gboolean
mono_unity_type_is_boolean (MonoType *type)
{
	return type->type == MONO_TYPE_BOOLEAN;
}

MonoClass *
mono_unity_type_get_element_class (MonoType *type)
{
...
MonoGenericContext
mono_unity_generic_class_get_context (MonoGenericClass *klass)
{
	return klass->context;
}

MonoClass *
mono_unity_generic_class_get_container_class (MonoGenericClass *klass)
{
...
MonoClass *
mono_unity_signature_get_class_for_param (MonoMethodSignature *sig, int index)
{
...
int
mono_unity_signature_num_parameters (MonoMethodSignature *sig)
{
	return sig->param_count;
}

gboolean
mono_unity_signature_param_is_byref (MonoMethodSignature *sig, int index)
{
...
guint
mono_unity_generic_inst_get_type_argc (MonoGenericInst *inst)
{
	return inst->type_argc;
}

MonoType *
mono_unity_generic_inst_get_type_argument (MonoGenericInst *inst, int index)
{
...
MonoString *
mono_unity_exception_get_message (MonoException *exc)
{
	return exc->message;
}

MonoString *
mono_unity_exception_get_stack_trace (MonoException *exc)
{
	return exc->stack_trace;
}

MonoObject *
mono_unity_exception_get_inner_exception (MonoException *exc)
{
	return exc->inner_ex;
}

MonoArray *
mono_unity_exception_get_trace_ips (MonoException *exc)
{
	return exc->trace_ips;
}

void
mono_unity_exception_set_trace_ips (MonoException *exc, MonoArray *ips)
{
...
MonoException *
mono_unity_exception_get_marshal_directive (const char *msg)
{
...
MonoClass *
mono_unity_defaults_get_int_class ()
{
	return mono_defaults.int_class;
}

MonoClass *
mono_unity_defaults_get_stack_frame_class ()
{
	return mono_defaults.stack_frame_class;
}

MonoClass *
mono_unity_defaults_get_int32_class ()
{
	return mono_defaults.int32_class;
}

MonoClass *
mono_unity_defaults_get_char_class ()
{
...
MonoClass *
mono_unity_defaults_get_byte_class ()
{
...
MONO_API void
mono_unity_domain_mempool_chunk_foreach (MonoDomain *domain, GFunc callback)
{
	mono_loader_lock ();
	mono_domain_lock (domain);

	mono_mempool_foreach_block (domain->mp, ReportMemPoolChunk, &callback);
...
MONO_API void
mono_unity_root_domain_mempool_chunk_foreach (GFunc callback)
{
...
MONO_API void
mono_unity_assembly_mempool_chunk_foreach (MonoAssembly *assembly, GFunc callback)
{
	//hold loader lock as we walk modules
	mono_loader_lock ();

	MonoImage *image = assembly->image;
	mono_mempool_foreach_block (image->mempool, ReportMemPoolChunk, &callback);
	...
	if (image->module_count > 0) {
		...
		for (i = 0; i < image->module_count; ++i) {
			...
			if (moduleImage) {
				mono_mempool_foreach_block (moduleImage->mempool, ReportMemPoolChunk, &callback);
			}
		}
	}
...
MonoImage *
mono_unity_image_get_mscorlib ()
{
	return mono_defaults.corlib->assembly->image;
}

MonoClass *
mono_unity_generic_container_get_parameter_class (MonoGenericContainer *generic_container, gint index)
{
...
MonoString *
mono_unity_string_append_assembly_name_if_necessary (MonoString *typeName, const char *assemblyName)
{
	if (typeName != NULL) {
...
MonoObject *
mono_unity_delegate_get_target (MonoDelegate *delegate)
{
	return delegate->target;
}

gchar *
mono_unity_get_runtime_build_info (const char *date, const char *time)
{
	return g_strdup_printf ("Unity IL2CPP(%s %s)", date, time);
}

void *
mono_unity_get_field_address (MonoObject *obj, MonoVTable *vt, MonoClassField *field)
{
...
gboolean
mono_unity_thread_state_init_from_handle (MonoThreadUnwindState *tctx, MonoThreadInfo *info, void *fixme)
{
	tctx->valid = TRUE;
...
void
mono_unity_stackframe_set_method (MonoStackFrame *sf, MonoMethod *method)
{
...
MonoType *
mono_unity_reflection_type_get_type (MonoReflectionType *type)
{
	return type->type;
}
